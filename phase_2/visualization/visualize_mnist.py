...
# NEW MAPPINGS (Matching dataset.py)
SUB_NAMES = [
...
def load_results(filename='results_mnist.json'):
    # Look in results folder relative to project root (assuming script is in visualization/)
...
def plot_boxplot(results):
    print("Generating Subcategory Box Plot...")
    ...
    # Sort by key info numerically
    sorted_keys = sorted(per_class_acc.keys(), key=lambda x: int(x))
    
    for key in sorted_keys:
        values = per_class_acc[key]
        if len(values) > 0: # Plot even if 0 accuracy to show it failed
    ...
    plt.figure(figsize=(12, 6)) # Slightly wider for more classes
    ...
    plt.title('MNIST Model: Accuracy per Subcategory')
    ...
    output_name = 'mnist_subcategory_boxplot.png'
...
def plot_group_boxplot(results):
    ...
    plt.title('MNIST Model: Accuracy per Broad Group')
    plt.ylabel('Accuracy Score')
    plt.xlabel('Broad Groups')
    plt.ylim(0.0, 1.05) 
    plt.tight_layout()
    
    output_name = 'mnist_group_boxplot.png'
    plt.savefig(output_name, dpi=300)
    print(f"Saved {output_name}")
...
def plot_bayesian(results, baseline=0.66):
    ...
    plt.plot(x, pdf, label='Posterior Distribution', color='#2ca02c', linewidth=2)
    plt.fill_between(x, pdf, color='#98df8a', alpha=0.4)

    plt.axvline(x=0, color='black', linestyle='--', alpha=0.5)

    plt.title(f'MNIST Model: Bayesian Posterior vs Baseline ({baseline*100:.1f}%)')
    ...
    plt.tight_layout()
    output_name = 'mnist_bayesian_posterior.png'
    plt.savefig(output_name, dpi=300)
    print(f"Saved {output_name}")

if __name__ == "__main__":
    results = load_results('results_mnist.json')
    if results:
